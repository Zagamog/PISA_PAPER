...
PISA_VNCO$NOREPEAT <- as.numeric(-(PISA_VNCO$REPEAT-1))



T1b <- PISA_VNCO[, c("VIETNAM","PRESCHOOL","REPEAT", "ST08Q01","ST09Q01","ST115Q01",
                    "OUTMATH","OUTREAD","OUTSCIE",
                    "PARPRESSURE","TIGERMOM","PROPCERT","SC35Q02",
                    "TCH_INCENTV","TCM_INSPE","COMP_USE","STU_FEEDB"
                    )]
...
results2 <- oaxaca(PV1MATH ~ PRESCHOOL+REPEAT+ST08Q01+ST09Q01+ST115Q01+
                  ...
                  + TCH_INCENTV+TCM_INSPE+COMP_USE+STU_FEEDB| OTHER,
                   data=PISA_VNCO2, R=2,reg.fun=Marek) 
plot(results2,
...
plot(results3,
     ...
     weight=0,title="Vietnam compared to Colombia (reading): Vietnam as reference",
     component.labels = c("explained"="xA-xB.BetaA", "unexplained"="xB.BetaA-BetaB")
)
...
plot(results4,
     ...
     weight=0,title="Vietnam compared to Colombia (science): Vietnam as reference",
     component.labels = c("explained"="xA-xB.BetaA", "unexplained"="xB.BetaA-BetaB")
)
